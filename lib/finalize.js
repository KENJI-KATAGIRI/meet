...
async function handleBniFinalize({ email, sessionId, staffName, memberName, bniContactId, durMin, transcript, summary }) {
  ...
  try {
    ...
    const driveSecret = process.env.DRIVE_INTERNAL_SECRET;
    if (!driveSecret) {
      console.warn('[INFO] DRIVE_INTERNAL_SECRET not set, skipping R2 backup');
    } else {
...
`);
  ...
  try {
    ...
    // BNI/福祉モードのベースプロンプト
    const basePrompt = isBniRecord
      ? 'BNIビジネス交流会の1対1ミーティングの日本語会話録音です。話者は2名のビジネスパーソンです。'
      : '日本語の業務会話録音です。';
    let prevChunkTail = ''; // 前チャンクの末尾テキスト（文脈引き継ぎ用）
    for (let ci = 0; ci < chunkFiles.length; ci++) {
      ...
      try {
        ...
        // 前チャンク末尾を文脈として付与（チャンク境界の誤認識を低減）
        const whisperPrompt = prevChunkTail
          ? basePrompt + ' ' + prevChunkTail.slice(-200)
          : basePrompt;
        ...
        if (whisperClient !== openai) transcribeParams.diarize = true; // Groq diarization
        const result = await whisperClient.audio.transcriptions.create(transcribeParams);
        const segs = result.segments || [];
        let skippedNoise = 0;
        for (const seg of segs) {
          const text = seg.text?.trim();
          // no_speech_prob > 0.6 のセグメントはノイズ・無音とみなしてスキップ
          if ((seg.no_speech_prob ?? 0) > 0.6) { skippedNoise++; continue; }
          if (text && !isWhisperHallucination(text)) {
            allSegments.push({ start: chunkOffset + seg.start, end: chunkOffset + seg.end, text, speaker: seg.speaker || null });
          }
        }
        console.log(`[audio-finalize] chunk ${f}: ${segs.length} segments, kept ${allSegments.length} total, noise-skipped=${skippedNoise}`);
        prevChunkTail = allSegments.slice(-3).map(s => s.text).join(' '); // 前チャンク末尾を次のprompt用に保存
      } catch(e) {
    ...
    // 話者分離：Groq diarizeラベル優先、なければギャップベースにフォールバック
    const turns = [];
    const hasGroqSpeakers = allSegments.some(s => s.speaker);
    if (hasGroqSpeakers) {
      // Groq が返す SPEAKER_00 / SPEAKER_01 を A / B にマッピング
      const speakerMap = {};
      ...
      for (const seg of allSegments) {
        ...
        if (!(spk in speakerMap)) speakerMap[spk] = labels[labelIdx++] || 'X';
        const label = speakerMap[spk];
        if (currentTurn && currentTurn.speaker === label) {
          currentTurn.text += ' ' + seg.text;
        } else {
          if (currentTurn) turns.push(currentTurn);
          currentTurn = { speaker: label, text: seg.text };
        }
      }
      if (currentTurn) turns.push(currentTurn);
      console.log('[audio-finalize] diarization: Groq speaker labels used, map:', JSON.stringify(speakerMap));
    } else {
      // フォールバック：無音ギャップ＋相槌長さで話者切替
      const AIZUCHI_DURATION = 1.0;
      ...
      for (const seg of allSegments) {
        ...
        if (gap > SPEAKER_CHANGE_GAP && currentTexts.length > 0 && duration > AIZUCHI_DURATION) {
      ...
      console.log('[audio-finalize] diarization: gap-based fallback used');
    }
    ...
    if (isBniRecord) {
      ...
      try {
        ...
        // BNI Managerのマイプロフィール GAINS（gains_* キー優先）
        ...
        } else if (pd.business_description || pd.selling_points || pd.category || pd.chapter) {
          // gains_*未設定の場合はprofile_dataの他フィールドでホスト情報を構築
          hostGains = {
            goals: '',
            accomplishments: pd.selling_points || '',
            interests: '',
            networks: pd.chapter || '',
            skills: [pd.category, pd.business_description].filter(Boolean).join(' / ')
          };
        }
      } catch(e) {}
