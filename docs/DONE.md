...
- [x] ウェイティングルーム（admit/reject）
  - 待機室ON/OFFは入室前プレビューで設定。承認は待機者がいる時だけ出る赤い「🙋 入室承認」ボタン
    （人数バッジ付き／待機者が出ると自動表示＋パネル自動オープン、全員さばくと自動で消える、2026-06-24）
- [x] チャット・ファイル送受信
- [x] 画面共有
  - 「参加者の画面共有を許可」をコントロールバーの独立トグルボタン化（ホスト/コホストのみ表示、2026-06-24）
    🔒画面共有:禁止 / 🔓画面共有:許可中（許可中は緑ハイライト）。🚪パネルからは撤去。
