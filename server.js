...
const { isWhisperHallucination, WELFARE_PROMPTS } = require('./lib/finalize');
const { enqueueFinalize } = require('./lib/queue');
...
db.pragma('journal_mode = WAL');   // ワーカープロセスとの同時アクセスのため
db.pragma('busy_timeout = 5000');  // 書き込み競合時に最大5秒待つ(SQLITE_BUSY回避)
db.exec(`
...
app.post('/api/upload-file', uploadLimiter, requireAuth, uploadFileMiddleware.single('file'), (req, res) => {
  ...
  const senderSocketId = typeof req.body.socketId === 'string' ? req.body.socketId.slice(0, 64) : null;
  if (roomId && rooms.has(roomId)) {
    const emit = senderSocketId ? io.to(roomId).except(senderSocketId) : io.to(roomId);
    emit.emit('chat-file', { from: senderName, message: msgHtml, time });
  }
  res.json({ ok: true, url, origName, isImage, msgHtml });
});
...
app.post('/api/audio-finalize', uploadLimiter, formParser, async (req, res) => {
  ...
  // 重い処理(文字起こし/要約/webhook)はワーカープロセスで実行。ここではジョブを永続キューに積むだけ。
  try {
    enqueueFinalize({
      sessionId, email, recordMode, welfareSystem, welfareRecordType,
      memberName, staffName, isWelfareRecord, isBniRecord, bniContactId,
      fUser: fUser || null,
    });
    console.log(`[audio-finalize] enqueued session=${sessionId}`);
  } catch(e) {
    console.error('[audio-finalize] enqueue error:', e.message);
  }
});

// NOTE: BNIモード/施設モードの finalize 処理は lib/finalize.js に移設（ワーカープロセスで実行）。
